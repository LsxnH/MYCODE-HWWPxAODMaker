...
# Install the project:
DESTDIR=${DESTDIR}/usr/HWWPhysicsxAODMaker/1.0.0 \
    make -k install/fast

# Remove the .dbg files from being packaged, in case we are in RelWithDebInfo
# build mode:
rm -f ${DESTDIR}/usr/HWWPhysicsxAODMaker/1.0.0/InstallArea/x86_64-slc6-gcc62-opt/bin/*.dbg
rm -f ${DESTDIR}/usr/HWWPhysicsxAODMaker/1.0.0/InstallArea/x86_64-slc6-gcc62-opt/lib/*.dbg
